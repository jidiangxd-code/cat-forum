...
exports.main = async (event, context) => {
  ...
  try {
    // 查询评论，验证身份
    const commentResult = await db.collection('comments').doc(trimmedCommentId).get();
    ...
    // 软删除评论
    await db.collection('comments').doc(trimmedCommentId).update({
      data: { status: 'deleted' }
    });
    ...
    try {
      if (comment.catId) {
        await db.collection('cats').doc(comment.catId).update({
