...
exports.main = async (event, context) => {
  ...
  if (!fromCatId || !toCatId) {
    return { success: false, code: 400, message: 'fromCatId 和 toCatId 不能为空' };
  }
  if (fromCatId === toCatId) {
  ...
  try {
    ...
    if (!fromCat) return { success: false, code: 404, message: '被合并的猫咪档案不存在' };
    if (!toCat) return { success: false, code: 404, message: '目标猫咪档案不存在' };
    if (fromCat.isMerged) return { success: false, code: 400, message: '该猫咪已经被合并过了' };
    if (toCat.isMerged) return { success: false, code: 400, message: '目标猫咪档案已被合并，请选择有效档案' };
    ...
    // 3. 旧猫票数累加到目标猫
    await db.collection('cats_profile').doc(toCatId).update({
      data: {
    ...
    // 4. 旧猫标记为已合并、隐藏
    await db.collection('cats_profile').doc(fromCatId).update({
      data: {
        isMerged: true,
        mergedTo: toCatId,
        updateTime: now,
